  ...
  const columns = [
    {
      ...
      renderCell: ({ row }) =>
        row.file_url ? (
          <img
            src={row.file_url.startsWith("data:") ? row.file_url : `${process.env.REACT_APP_API_URL || ""}${row.file_url}`}
            alt={row.alt_text || row.year}
            className="h-10 w-10 object-cover rounded border p-1"
          />
        ) : (
        ...
        <div className="flex items-center gap-2">
          {hasPermission("about", "update") && (
            <Button
              ...
              onClick={() => navigate(`/about-gdb/journey-timeline/edit/${row.id}`)}
            >
