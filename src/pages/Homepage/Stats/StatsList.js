...
import { getStats, updateStat } from "../../../services/homepage";
...
export default function StatsList() {
  ...
  const handleToggleStatus = async (row) => {
    try {
      const res = await updateStat(row.id, {
        ...row,
        ...
        <div className="flex items-center gap-2">
          {hasPermission("homepage", "stats.update") && (
            <>
              ...
              <StatusActionButton row={row} entityName="stat" onConfirm={handleToggleStatus} />
            </>
          )}
        </div>
      ),
