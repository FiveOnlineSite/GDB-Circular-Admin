...
import { Plus, Edit2, Trash2, Save, Loader2, ShieldCheck } from "lucide-react";
...
import {
  ...
  getQualityAssuranceCards,
  deleteQualityAssuranceCard,
...
export default function LogisticsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await getQualityAssuranceSection();
      ...
      toast.error("Failed to load quality assurance section");
      setIsEditingSection(true);
    } finally {
      setLoading(false);
    }
  ...
  const reloadCards = async () => {
    try {
      const res = await getQualityAssuranceCards();
      if (res.success) {
  ...
  const handleSectionSave = async (e) => {
    ...
    try {
      ...
      const res = await updateQualityAssuranceSection(section);
      if (res.success) {
  ...
  const handleDelete = async () => {
    try {
      const res = await deleteQualityAssuranceCard(selected.id);
      if (res.success) {
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          {hasPermission("product", "quality.delete") && (
            <Button
              variant="outline"
              size="sm"
