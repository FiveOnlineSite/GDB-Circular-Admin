...
export default function SellerLogisticsCardFormPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setPageLoading(true);
        const res = await getSellerLogisticsCardById(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = isEdit ? await updateSellerLogisticsCard(id, payload) : await createSellerLogisticsCard(payload);
      if (res.success) {
  ...
  return (
    <div className="space-y-6 pb-12 w-full">
      <div className="flex items-center gap-3">
        <Button variant="outline" size="icon" type="button" onClick={() => navigate("/sellers/logistics")} className="rounded-xl border-slate-200">
