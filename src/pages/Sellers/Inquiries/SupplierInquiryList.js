...
export default function SupplierInquiryList() {
  ...
  const fetchInquiries = useCallback(async (params = {}) => {
    try {
      ...
      if (activeStartDate) queryParams.startDate = `${activeStartDate} 00:00:00`;
      if (activeEndDate) queryParams.endDate = `${activeEndDate} 23:59:59`;
    ...
    try {
      ...
      if (res.success && res.data) {
        const formatted = res.data.map((item) => ({
          ...
          "Resin Type": item.resin_type || "N/A",
          "Volume Required": item.volume || "N/A",
  ...
  const columns = [
    { field: "id", headerName: "ID", sortable: true },
    {
      ...
      sortable: true,
      renderCell: ({ row }) => <span className="font-semibold text-slate-800">{row.name}</span>,
    },
    ...
    },
    {
      ...
      sortable: true,
      renderCell: ({ row }) => <span className="px-2 py-0.5 rounded text-xs font-semibold bg-sky-50 text-sky-700 border border-sky-100">{row.resin_type || "N/A"}</span>,
    },
    { field: "timeline", headerName: "Timeline", sortable: false, renderCell: ({ row }) => row.timeline || "-" },
    {
      field: "created_at",
      headerName: "Date Submitted",
      sortable: true,
      renderCell: ({ row }) => <span className="text-xs text-slate-500">{row.created_at ? new Date(row.created_at).toLocaleDateString() : "-"}</span>,
    },
    {
      field: "actions",
      headerName: "Actions",
    ...
    <div className="p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        {hasPermission("sellers", "inquiry.export") && (
          <Button onClick={handleExcelExport} disabled={exporting} className="bg-[#981B1F] hover:bg-[#C3662D] text-white shadow-sm transition-colors self-start md:self-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <Input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPagination((p) => ({ ...p, current_page: 1 })); }} placeholder="Search by name, email, company..." className="h-10 border-[#E6E6E6] bg-white pl-10 pr-3 text-sm" />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 block mb-1">Resin Type</label>
            <select value={selectedResin} onChange={(e) => { setSelectedResin(e.target.value); setPagination((p) => ({ ...p, current_page: 1 })); }} className="w-full border border-[#E6E6E6] rounded-lg p-2 text-sm focus:border-[#981B1F] focus:outline-none bg-white text-slate-700 h-10 cursor-pointer">
              <option value="">All Resin Types</option>
              {RESIN_TYPES.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
            </select>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type="date" value={startDate} onChange={(e) => { setStartDate(e.target.value); setPagination((p) => ({ ...p, current_page: 1 })); }} aria-label="Received From" className="inquiry-date-input absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="date" value={endDate} onChange={(e) => { setEndDate(e.target.value); setPagination((p) => ({ ...p, current_page: 1 })); }} aria-label="Received To" className="inquiry-date-input absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" />
              <CalendarDays className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
            </div>
          </div>
        </div>
        <div className="flex justify-end">
          <Button type="button" variant="ghost" onClick={handleResetFilters} className="text-xs text-slate-500 hover:text-[#981B1F]">Clear All Filters</Button>
        </div>
      </div>

      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
        <ReusableDataTable columns={columns} rows={rows} loading={loading} pagination={pagination} handlePageChange={(p) => setPagination((prev) => ({ ...prev, current_page: p }))} handlePerPageChange={(pp) => setPagination((prev) => ({ ...prev, per_page: pp, current_page: 1 }))} emptyMessage="No supplier inquiries match the selected criteria." />
      </div>
